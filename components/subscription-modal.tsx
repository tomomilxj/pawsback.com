...
export function SubscriptionModal({ isOpen, onClose }: SubscriptionModalProps) {
  ...
  const t = translations[language].subscriptionModal
          ...
          >
            <div className="bg-card/80 backdrop-blur-2xl border border-border/40 rounded-3xl shadow-2xl max-w-md w-full p-8 pointer-events-auto relative max-h-[90vh] overflow-y-auto">
              ...
              <div className="text-center mb-8">
                ...
                <h2 className="text-3xl font-light text-foreground mb-2 text-balance">{t.title}</h2>
                <p className="text-muted-foreground font-light text-pretty">{t.subtitle}</p>
                <p className="text-sm text-muted-foreground/80 mt-2 text-pretty">{t.description}</p>
              </div>

              {/* Waveform comparison */}
              <div className="mb-8 grid grid-cols-2 gap-4">
                <div className="p-4 bg-foreground/5 rounded-2xl">
                  <p className="text-xs text-muted-foreground mb-3 text-center">{t.basicVersion}</p>
                  <div className="flex items-center justify-center gap-1 h-16">
                    {[...Array(12)].map((_, i) => (
                      <div key={i} className="w-1 bg-foreground/20 rounded-full" style={{ height: "8px" }} />
                    ))}
                  </div>
                  <p className="text-xs text-muted-foreground mt-2 text-center">{t.genericSounds}</p>
                </div>

                <div className="p-4 bg-foreground/10 rounded-2xl border border-foreground/20">
                  <p className="text-xs text-foreground mb-3 text-center font-medium">{t.premiumVersion}</p>
                  <div className="flex items-center justify-center gap-1 h-16">
                  ...
                  <p className="text-xs text-foreground mt-2 text-center font-medium">{t.theirVoice}</p>
                </div>
              </div>

              {/* Features */}
              <div className="space-y-4 mb-8">
                <Feature icon={Volume2} title={t.feature1Title} description={t.feature1Desc} />
                <Feature icon={Star} title={t.feature2Title} description={t.feature2Desc} />
                <Feature icon={Sparkles} title={t.feature3Title} description={t.feature3Desc} />
              </div>

              {/* Video upload section */}
              <div className="mb-6">
                <label className="block text-sm text-muted-foreground mb-3 text-center">{t.uploadLabel}</label>
                  ...
                  >
                    {uploadedFile ? (
                      ...
                      <>
                        ...
                        <p className="text-sm text-foreground">{t.uploadButton}</p>
              ...
              >
                {isProcessing ? t.processing : t.upgradeButton}
              </motion.button>

              <p className="text-xs text-muted-foreground text-center mt-4">{t.trialInfo}</p>
            </div>
          </motion.div>
        </>
      )}
